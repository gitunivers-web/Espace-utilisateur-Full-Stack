...
/**
 * Script de seed complet pour les types de prêts avec données réalistes (2024-2025)
 * Basé sur les taux actuels du marché français
 */
...
async function seed() {
  try {
    ...
    console.log(`   - Particuliers : ${loanTypesData.filter(l => l.category === "particular").length}`);
    console.log(`   - Professionnels : ${loanTypesData.filter(l => l.category === "professional").length}`);
